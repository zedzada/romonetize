  ...
  try {
    ...
    for (const game of games) {
      try {
        ...
        // Insert CCU snapshot - use only columns that exist in schema
        // Schema has: id, game_id, ccu, created_at
        // Note: We always insert, never skip - even if CCU is same as previous
        const { error: insertError } = await supabase
          .from("ccu_snapshots")
          .insert({
            game_id: game.id,
            ccu: stats.currentPlayers,
            // created_at is auto-generated by default, but we set it explicitly for clarity
